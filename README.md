...
3. Una **API key de Anthropic**: https://console.anthropic.com/settings/keys
...
Copiá `.env.example` como `.env` y completá tu API key:
...
- El asistente solo puede ejecutar las acciones definidas en `tools.py`; no
  ...
  proyectos, no a `C:\`.
- Tu `.env` con la API key nunca se debe subir a un repositorio público.
